	...
	var Home = function (_Component) {
	  ...
	  function Home() {
	    ...
	    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Home.__proto__ || Object.getPrototypeOf(Home)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
	      ...
	      _this.setState({ foundedItems: null });
	      ...
	      var search = null; //this.state.titles;
	      var searchString = _this.state.searchString.trim().toLowerCase();
	
	      if (searchString.length > 0) {
	        search = _this.state.posts.filter(function (item) {
	          if (item.title.toLowerCase().match(searchString)) return item;
	        });
	      }
	      _this.setState({ foundedItems: search });
	  ...
	  _createClass(Home, [{
	    ...
	    value: function componentDidMount() {
	      ...
	      _axios2.default.get('http://api.themoviedb.org/3/movie/popular?api_key=49a5dbae3f8c8632aba8f07513a7cbb2&language=ru-RU&page=1').then(function (res) {
	        var _titles = [].concat(_toConsumableArray(_this2.state.posts), _toConsumableArray(res.data.results)).map(function (item) {
	          return item.title;
	        });
